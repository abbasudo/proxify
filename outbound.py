...
def convert_uri_reality_json(uri):
    ...
    if "host=" in uri:
        ...
        data['streamSettings'].update(headers)
...
def convert_uri_vless_ws_json(uri):
    protocol = uri.split("://")[0]
    uid = uri.split("//")[1].split("@")[0]
    address = uri.split('@')[1].split(":")[0]
    destination_port = int(uri.split(address + ':')[1].split("?")[0])
...
def convert_uri_vless_tcp_json(uri):
    protocol = uri.split("://")[0]
    uid = uri.split("//")[1].split("@")[0]
    address = uri.split('@')[1].split(":")[0]
    destination_port = int(uri.split(address + ':')[1].split("?")[0])
...
def convert_uri_vmess_ws_json(uri):
...
def convert_uri_vmess_tcp_json(uri):
    decoded = json.loads(base64.b64decode(uri.split("://")[1]).decode())
...
def convert_uri_trojan_reality_json(uri):
    protocol = uri.split("://")[0]
...
def convert_uri_trojan_ws_json(uri):
    protocol = uri.split("://")[0]
    password = uri.split("//")[1].split("@")[0]
    address = uri.split('@')[1].split(":")[0]
    destination_port = int(uri.split(address + ':')[1].split("?")[0])
...
def convert_uri_trojan_tcp_json(uri):
    protocol = uri.split("://")[0]
    password = uri.split("//")[1].split("@")[0]
    address = uri.split('@')[1].split(":")[0]
    destination_port = int(uri.split(address + ':')[1].split("?")[0])
...
def generate(uri, tag):
    uri = urllib.parse.unquote(uri)

    if Vless_Reality_checker(uri) is True:
        file = convert_uri_reality_json(uri)
    elif vless_ws_checker(uri) is True:
        file = convert_uri_vless_ws_json(uri)
    elif vless_tcp_checker(uri) is True:
        file = convert_uri_vless_tcp_json(uri)
    elif vmess_ws_checker(uri) is True:
        file = convert_uri_vmess_ws_json(uri)
    elif vmess_tcp_checker(uri) is True:
        file = convert_uri_vmess_tcp_json(uri)
    elif trojan_Reality_checker(uri) is True:
        file = convert_uri_trojan_reality_json(uri)
    elif trojan_ws_checker(uri) is True:
        file = convert_uri_trojan_ws_json(uri)
    elif trojan_tcp_checker(uri) is True:
        file = convert_uri_trojan_tcp_json(uri)
